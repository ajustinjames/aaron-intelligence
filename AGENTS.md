...
- **Everything in this repo is published.** It's a public marketplace, so every skill, README, and commit message is world-readable, and so is any PR description. Never bake in findings from a live audit of the owner's own repos — repo names, which repos are misconfigured, absolute paths containing a username, org-internal hostnames. Write the *rule* and the command that detects it, never the *result* of having run it. This is a real regression that shipped once: `repo-standards` named two repos and disclosed that their code-owner rule was unenforced.
