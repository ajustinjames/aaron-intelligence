#!/usr/bin/env node
// caveman-lite — shared configuration resolver
//
...
//   3. 'full'
//
// No agent/command/skill modes here (commit, review, compress) — caveman-lite
// ships hooks only, so there's nothing installed to back those modes.
...
// Symlink-safe flag file write. Uses O_NOFOLLOW where available, writes
// atomically via temp + rename with 0600 permissions. Protects against a
// local attacker replacing the predictable flag path
// (~/.claude/.caveman-lite-active) with a symlink to clobber other files.
...
// Symlink-safe, size-capped, whitelist-validated flag file read. Returns null
// on any anomaly — never inject untrusted bytes into model context.
