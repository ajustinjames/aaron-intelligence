...
All six roles are **leaf agents** — they must never spawn subagents of their own. Since plugin `1.3.0` this is enforced at the tool level, not just by prompt: `scout`/`Explore` have a `tools:` allowlist that omits the spawn tools, and the other four carry `disallowedTools: Task, Agent, Workflow` in their frontmatter (a local hardening on top of upstream `1.1.1`, which relied on the main-session policy text alone — text that subagents never even see, since the hooks below fire only in the main session). Each role's system prompt also states the leaf rule, as defense in depth.
