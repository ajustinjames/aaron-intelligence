...
- `agents/verifier.md` — adversarial fresh-context verification, read-and-run only: its `tools:` allowlist is `Read, Bash, Grep, Glob` (`opus`).
...
All six roles are **leaf agents** — they must never spawn subagents of their own. Since plugin `1.5.0` this is enforced at the tool level via a `tools:` allowlist on every role's frontmatter (a local hardening on top of upstream `1.1.1`, which relied on the main-session policy text alone — text that subagents never even see, since the hooks below fire only in the main session). An allowlist fails closed: any future subagent-spawning or messaging tool is excluded by default, unlike the `disallowedTools` denylist used in plugin `1.3.0`–`1.4.0`. Each role's system prompt also states the leaf rule, as defense in depth.
